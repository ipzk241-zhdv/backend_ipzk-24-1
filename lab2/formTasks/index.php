...
foreach ($_SESSION as $key => $value) { // перебрати ключі і значення масиву _POST
    if ($key == "PHPSESSID") {
        continue;
    }
    ...
    if (strlen($_SESSION[$key]) > 0) { // якщо довжина значення ключа більша за 0 (дефакто, встановлена)
        $$key = $_SESSION[$key]; // створити змінну з назвою ключа і її значенням
    }
...
<body>
    ...
    <?php
    if ($isLanguage) {
        echo "<p><b>Обрана мова: $language</b></p>";
    }
    ?>
    <form method="POST" action="./result.php" enctype="multipart/form-data">
        <table>
            <tr>
                <td>Логін:</td>
                <td><input type="text" name="login" value="<?= isset($login) ? $login : "" ?>"></td>
            </tr>
            <tr>
                <td>Пароль:</td>
            ...
            <tr>
                ...
                <td><input type="password" name="passwordCheck"
                        value="<?= isset($passwordCheck) ? $passwordCheck : "" ?>"></td>
            </tr>
            <tr>
                <td>Стать:</td>
                <td>
                    <input type="radio" name="gender" value="male" <?= isset($gender) ? $gender == "male" ? "checked" : "" : "" ?>>Чоловік
                    <input type="radio" name="gender" value="female" <?= isset($gender) ? $gender == "female" ? "checked" : "" : "" ?>>Жінка
                </td>
            </tr>
            <tr>
                <td>Місто:</td>
                <td>
                    <select name="city">
                        <option value="Житомир" <?= isset($city) ? $city == "Житомир" ? "selected" : "" : "" ?>>Житомир
                        </option>
                        <option value="Київ" <?= isset($city) ? $city == "Київ" ? "selected" : "" : "" ?>>Київ</option>
                        <option value="Львів" <?= isset($city) ? $city == "Львів" ? "selected" : "" : "" ?>>Львів</option>
                        <option value="Суми" <?= isset($city) ? $city == "Суми" ? "selected" : "" : "" ?>>Суми</option>
                        <option value="Харків" <?= isset($city) ? $city == "Харків" ? "selected" : "" : "" ?>>Харків
                        </option>
                    </select>
                </td>
            </tr>
            <tr>
                <td>Улюблені ігри:</td>
                <td><input type="checkbox" name="games[]" value="football" <?= isset($games) ? in_array("football", $games) ? "checked" : "" : "" ?>>Футбол</td>
            </tr>
            <tr>
                <td></td>
                <td><input type="checkbox" name="games[]" value="basketball" <?= isset($games) ? in_array("basketball", $games) ? "checked" : "" : "" ?>>Баскетбол</td>
            </tr>
            <tr>
                <td></td>
                <td><input type="checkbox" name="games[]" value="volleyball" <?= isset($games) ? in_array("volleyball", $games) ? "checked" : "" : "" ?>>Волейбол</td>
            </tr>
            <tr>
                <td></td>
                <td><input type="checkbox" name="games[]" value="chess" <?= isset($games) ? in_array("chess", $games) ? "checked" : "" : "" ?>>Шахи</td>
            </tr>
            <tr>
                <td></td>
                <td><input type="checkbox" name="games[]" value="world of tanks" <?= isset($games) ? in_array("world of tanks", $games) ? "checked" : "" : "" ?>>World of Tanks</td>
            </tr>
            <tr>
                <td>Про себе:</td>
                <td><textarea name="info" cols="20" rows="10"><?= isset($info) ? $info : "" ?></textarea></td>
            </tr>
            <tr>
                <td>Фотографія:</td>
                <td><input type="file" accept="image/png" name="userPhoto" id="userPhoto"></td>
            </tr>
            <tr>
                <td></td>
