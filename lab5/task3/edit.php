    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if (!$error) {
            ...
            if (!$missing) {
                ...
                if ($employee === false) { // якщо запит повернув false, продукту з таким id немає
                    $wrong = true;
                } else {
                    $canChange = [ // ассоціативний масив для динамічного створення таблиці з можливостю редагування продукту
                        "id" => 'type="number" name="id" readonly value="' . $employee['id'] . '"',
                        "name" => 'type="text" name="name" value="' . $employee['name'] . '"',
                        "position" => 'type="text" name="position" value="' . $employee['position'] . '"',
                        "salary" => 'type="number" min="1" name="salary" step="0.1" value="' . $employee['salary'] . '"',
                    ];
                }
            }
        }
        ...
        <?php else: ?>
            <form method="POST" action="">
                <table>
                    <tr>
                        <?php
                        foreach ($employee as $key => $value) { // відображення назв всіх полів таблиці з бд
                            echo "<td>$key</td>";
                        }
                        ?>
                    </tr>
                    <tr>
                        <?php
                        foreach ($employee as $key => $value) { // відображення значень ціх полів
                            echo "<td>$value</td>";
                        }
                        ?>
                    </tr>
